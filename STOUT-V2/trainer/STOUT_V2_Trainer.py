...
def get_training_dataset(batch_size = BATCH_SIZE,buffered_size = BUFFER_SIZE):
	...
	filenames = sorted(tf.io.gfile.glob( "gs://tpu-test-koh/STOUT_V2/STOUT_V2_development/1mio_SMI_character/TF_rec/*.tfrecord"), key=numericalSort)
...
with strategy.scope():
	...
	validation_accuracy = tf.keras.metrics.SparseCategoricalAccuracy(name='validation_accuracy', dtype=tf.float32)

	#Initialize Transformer
	transformer = nmt_model_transformer.Transformer(num_layer,d_model,num_heads,dff,input_vocab_size,target_vocab_size,inp_max_length,targ_max_length,rate=dropout_rate)


checkpoint_path = 'gs://tpu-test-koh/STOUT_V2/STOUT_V2_development/1mio_SMI_character/checkpoints/'
...
def train_step(iterator):
	def step_fn(inputs):
		...
		with tf.GradientTape() as tape:
			...
			train_accuracy.update_state(tar_real, predictions, sample_weight=tf.where(tf.not_equal(tar_real, PAD_TOKEN), 1.0, 0.0))
	...
	def step_fn(inputs):
		...
		with tf.GradientTape() as tape:
			predictions, _ = transformer(inp, tar_inp,
                                 ...
                                 dec_padding_mask)
			loss = loss_function(tar_real, predictions)
			validation_accuracy.update_state(tar_real, predictions, sample_weight=tf.where(tf.not_equal(tar_real, PAD_TOKEN), 1.0, 0.0))
...
for epoch in range(start_epoch, EPOCHS):
	...
	for x in train_dataset:
		...
		if batch % 100 == 0:
			print ('Epoch {} Batch {} Loss {:.4f} Accuracy {:.4f}'.format(epoch + 1, batch,train_loss.result(),train_accuracy.result()), flush=True)
		...
		if batch == num_steps:
			...
			print ('Epoch {} Training_Loss {:.4f} Accuracy {:.4f}'.format(epoch + 1, train_loss.result(), train_accuracy.result()), flush=True)
